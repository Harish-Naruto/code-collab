...
const CodeCollabPage: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? "bg-gray-900 text-white" : "bg-white text-black"} flex flex-col transition-colors`}>
      
      {/* Header */}
      <header className={`${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-gray-200 border-gray-300"} border-b px-4 py-2 flex items-center justify-between`}>
      ...
      <div className="flex flex-1">
        ...
        <Editor content={editorContent} onChange={setEditorContent} fileName={selectedFile} />
        <Chat />
      </div>
    </div>
  )
